...
<div class="judul"><h3>Surat Pengantar SKCK</h3><p>Nomor: {{ $surat->nomor_surat }}</p></div>
<div class="isi">
    <p>Yang bertanda tangan di bawah ini, Lurah {{ $setting->nama_kelurahan }}, dengan ini mengantarkan permohonan Surat Keterangan Catatan Kepolisian (SKCK) atas nama:</p>
    <table class="data" style="margin-top:6px;">
        ...
        <tr><td class="label">Keperluan</td><td class="sep">:</td><td class="value">{{ $extra['keperluan'] ?? '-' }}</td></tr>
        @if(!empty($extra['tujuan_instansi']))<tr><td class="label">Ditujukan ke</td><td class="sep">:</td><td class="value">{{ $extra['tujuan_instansi'] }}</td></tr>@endif
    </table>
    <p style="margin-top:6px;">Yang bersangkutan adalah warga Kalurahan {{ $setting->nama_kelurahan }} yang berkelakuan baik dan tidak pernah terlibat tindak pidana.</p>
</div>
